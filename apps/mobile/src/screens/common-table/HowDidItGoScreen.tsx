      ...
      const finishResults =
        finishMembers.length > 0
          ? finishMembers.map((finish) => ({
              ...
              status:
                personChoices[finish.memberId] === 'skipped'
                  ? ('skipped' as const)
                  : ('applied' as const),
            }))
          : undefined;
      await completeMeal(sharedMealId, { finishResults });
      const memberOutcomes = finishMembers.flatMap(
        ...
        {finishMembers.length > 0 && (
          <>
            ...
            <View style={styles.memberList}>
              {finishMembers.map((finish) => {
                const choice =
                  personChoices[finish.memberId] ?? (finish.title ? 'worked' : undefined);
                return (
